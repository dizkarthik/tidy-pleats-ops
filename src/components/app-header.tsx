...
import { ArrowLeft, KeyRound, LogOut, Menu, Pencil, X } from "lucide-react";
...
export function AppHeader({ user, title, backHref, editHref }: AppHeaderProps) {
  void user;
    ...
    <>
      <header className="sticky top-0 z-30 border-b border-stone-200 bg-white/95 backdrop-blur">
        <div className="mx-auto grid h-16 max-w-5xl grid-cols-[2.5rem_1fr_2.5rem] items-center gap-3 px-4">
          ...
          {title ? (
            <p className="min-w-0 truncate text-center text-sm font-bold text-stone-950">
              {title}
            </p>
          ) : (
